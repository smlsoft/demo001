...
export default function DocsExpensePage() {
  ...
  async function upload(e: React.FormEvent) {
    ...
    fd.append("category", "เอกสารเงินออก");
    fd.append("description", desc || "เอกสารรายจ่าย");
    ...
    <div className="space-y-4 lg:max-w-4xl lg:mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>ใบเสร็จจ่ายเงิน บิล ใบกำกับภาษี หลักฐานรายจ่าย</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="px-4 py-2 rounded-xl text-white font-medium" style={{ background: "var(--expense)" }}>
          + อัปโหลด
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={upload} className="card space-y-3 lg:max-w-xl">
          ...
          <input type="text" value={desc} onChange={(e) => setDesc(e.target.value)}
            placeholder="คำอธิบาย เช่น ใบเสร็จค่าปุ๋ย, บิลค่าไฟฟ้า"
            className="w-full border rounded-xl px-4 py-3" />
          <div className="flex gap-2 flex-wrap text-xs" style={{ color: "var(--text-muted)" }}>
            {["ใบเสร็จค่าปุ๋ย", "บิลค่าไฟฟ้า", "บิลค่าน้ำ", "ใบเสร็จซ่อมรถ", "ใบเสร็จค่ายา", "ใบกำกับภาษี", "สัญญากู้เงิน"].map((s) => (
          ...
          <button type="submit" disabled={uploading} className="w-full py-3 rounded-xl text-white font-bold"
            style={{ background: "var(--expense)", opacity: uploading ? 0.5 : 1 }}>
            {uploading ? "กำลังอัปโหลด..." : "📤 อัปโหลดเอกสารเงินออก"}
          </button>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>อัปโหลดใบเสร็จ บิลค่าใช้จ่าย หลักฐานรายจ่ายต่างๆ</p>
        </div>
      ) : (
            ...
            <div key={f._id} className="card flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <a href={`/api/files/${f._id}`} target="_blank" rel="noopener noreferrer"
                  className="font-medium truncate block" style={{ color: "var(--expense)" }}>
                  {f.originalName}
                </a>
