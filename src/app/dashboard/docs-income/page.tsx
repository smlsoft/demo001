  ...
  return (
    <div className="space-y-4 lg:max-w-4xl lg:mx-auto">
      <div>
        ...
        <p className="text-sm" style={{ color: "var(--text-muted)" }}>ใบเสร็จรับเงิน สัญญา หลักฐานรายรับ — ส่งรูปผ่าน Telegram จะแสดงที่นี่</p>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {files.map((f) => (
            <a key={f._id} href={`/api/files/${f._id}`} target="_blank" rel="noopener noreferrer"
              ...
              style={{ borderLeft: "3px solid var(--income)" }}>
              {/* Thumbnail */}
